...
import { NCard, NInput, NInputNumber, NSelect, NSwitch, NTag } from 'naive-ui';
...
/* ============================================================
 * 字典项表单（kind='data'）：手写三段 NCard（视觉对齐 react-admin）
 *
 * 不走 vben Form，因为 vben Form 的字段是顺序渲染到单个 wrapperClass
 * 网格里，无法拆到不同 Card 容器。手写 NCard + reactive 让三段卡片
...
// （在 Naive UI 白名单外但非空 / 非 'default'，如历史写入的 magenta / processing）
// 时为 true，让预览区下方显示一行小字告知用户保存后会丢颜色信息。
...
/** 预览颜色：usePresetStyle=false → 'default'（Naive UI 无色态）；
 *  开启时直接传 dataModel.tagType，不再走 TAG_TYPE_TO_EL_TYPE 多对一压扁 */
const previewElType = computed<
  'default' | 'error' | 'info' | 'primary' | 'success' | 'warning'
>(() => {
  if (!dataModel.usePresetStyle) return 'default';
  return dataModel.tagType as
    | 'default'
    | 'error'
    | 'info'
    | 'primary'
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  async onConfirm() {
    ...
    if (props.kind === 'type') {
      ...
      // 提交归一化（design §6 / implement §3）：永远下发白名单内的值。
      // usePresetStyle=false → 'default'（避免后端 PUT 对空串 400）；
      // usePresetStyle=true 时若当前 tagType 不在 Naive UI 白名单（理论上不应发生，
      // 但防御性归一化），强制回退到 'primary'。前端白名单 ⊂ 后端 16 项白名单。
      let tagType: string;
      if (!dataModel.usePresetStyle) {
        tagType = 'default';
      } else if (NAIVE_TAG_TYPE_SET.has(dataModel.tagType)) {
...
<template>
  <Drawer :title="drawerTitle">
    ...
    <!--
      字典项（kind='data'）：三段独立 NCard，分组视觉对齐 react-admin。
    ...
    <div v-else class="dict-data-form flex flex-col gap-4">
      ...
      <NCard :bordered="true" size="small" title="基础信息">
        <div class="grid grid-cols-1 gap-x-4 gap-y-4 md:grid-cols-2">
          <div class="col-span-2">
            ...
            <NSelect
              v-model:value="dataModel.typeId"
      ...
      <!-- ============ 样式设置 ============ -->
      <NCard :bordered="true" size="small" title="样式设置">
        <!--
        ...
        >
          <div class="md:col-span-3">
            ...
            <NSwitch
              v-model:value="dataModel.usePresetStyle"
          ...
          >
            ...
            <!--
              legacy tag_type 提示：编辑历史 Naive UI 不识别的颜色预设
              （如 magenta）时被设为 true，提示用户保存后视觉会丢失。
              仅在命中 legacy 时显示，不抢视觉位。
            -->
      ...
      <!-- ============ 其他属性 ============ -->
      <NCard :bordered="true" size="small" title="其他属性">
        <div class="grid grid-cols-1 gap-x-4 gap-y-4 md:grid-cols-2">
          <div>
          ...
          <div>
            ...
            <NSwitch v-model:value="dataModel.isDefault">
          ...
          <div>
            ...
            <NSwitch v-model:value="dataModel.isEnabled">
